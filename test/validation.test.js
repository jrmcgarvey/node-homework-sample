const { userSchema, patchUserSchema } = require("../validation/userSchema")
const { taskSchema, patchTaskSchema } = require("../validation/taskSchema")

test("user object validation", () => {
    let { error,value} = userSchema.validate({name: 'a', email: 'b', password: 'c' },  {abortEarly: false})
    ...
    ;({error, value} = userSchema.validate({name: '  Bob  ', email: 'bob@sample.com', password: 'Pa$$word20'}))
    expect(error).toBeUndefined()
    expect(value.name).toBe('Bob')
})

test("task object validation", () => {
    let {error, value} = taskSchema.validate({title: 7, isCompleted: 'baloney'}, {abortEarly: false})
    expect(error).toBeDefined()
